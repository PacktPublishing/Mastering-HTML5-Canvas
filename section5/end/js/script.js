(function(){
	...
	function doCanvas(){
		...
		createRadialGradient(createCavnas(200,125));	
		createFilteredText(createCavnas(200,125));	
		createPixels(createCavnas(200,125));
		createClip(createCavnas(200,125));

		createFin(byId('myCanvas'));	
	}
...
function createRadialGradient(canvas){
	...
	function createFilteredText(canvas){
		...
		do{
			...
			tbox = context.measureText(text);

		}while(tbox.width <w*.9);
	...
	function createPixels(canvas){
		...
		var img = new Image();
				img.onload = function(e){
					...
					var imageData = context.getImageData(0,0,w/2,h),
							pixelData = imageData.data,
							pixelLen = pixelData.length;

					for(var i = 0; i<pixelLen; i+=4){
						pixelData[i] = 255-pixelData[i];//RR
						pixelData[i+1] = 255-pixelData[i+1];//GG
						pixelData[i+2] = 255-pixelData[i+2];//BB

					}
					context.putImageData(imageData,0,0);
				};
				img.src = 'img/js-sorry.jpg';

	}

	function generatePattern(context,w,h,size,offset){
		for(var i=0; i<w; i+=2){
			for(var j=0; j<h; j++){
				context.rect((i+(j+offset)%2)*size,j*size,size,size);
			}
		}
	...
	function createClip(canvas){
		var w = canvas.width,
				...
				context = canvas.getContext('2d'),
				size = 20,
				iw = Math.floor(w/size) + 1,
				ih = Math.floor(h/size) + 1;

		context.save();

		context.beginPath();
		context.fillStyle = '#ffffff';
		generatePattern(context,iw,ih,size,1);
		context.closePath();
		context.fill();

		context.transform(1.2,.2,.2,1.3,-35,0);
		...
		context.beginPath();
		context.fillStyle = '#000000';
		generatePattern(context,iw,ih,size,0);
		context.closePath();
		context.fill();
	...
	function createFin(canvas){
		...
		context.fillStyle = '#000000';
		context.fillText('Goodbye.',w/2,h/2+46);
